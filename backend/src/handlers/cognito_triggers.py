...
users_table = dynamodb.Table(os.environ.get('USERS_TABLE_NAME', 'meeting-agent-users'))
...
def post_confirmation_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        create_default_preferences(user_id, timezone)
...
def create_default_preferences(user_id: str, timezone: str) -> None:
    """
    ...
    """
    try:
        preferences_table = dynamodb.Table(os.environ.get('PREFERENCES_TABLE_NAME', 'meeting-agent-preferences'))
